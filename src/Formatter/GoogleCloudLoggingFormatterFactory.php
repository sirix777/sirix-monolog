...
class GoogleCloudLoggingFormatterFactory implements FormatterFactoryInterface
{
    public function create(ContainerInterface $container, FormatterDefinition $formatterDefinition): GoogleCloudLoggingFormatter
    {
        ...
        if (JsonFormatter::BATCH_MODE_JSON !== $batchMode && JsonFormatter::BATCH_MODE_NEWLINES !== $batchMode) {
            throw new InvalidConfigException('Google Cloud Logging formatter option "batch_mode" must be a valid JsonFormatter batch mode.');
        }

        return new GoogleCloudLoggingFormatter(
            $batchMode,
